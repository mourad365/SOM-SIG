...
// attendu_kw ≈ n_clients_du_transfo × médiane(puiss_souscrite du réseau)
...
analyticsRouter.get('/pertes', async (_req, res) => {
  try {
    const { rows } = await query(
      `WITH med AS (
         SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY puiss_souscrite_kw) AS kw
         FROM point_service
         WHERE puiss_souscrite_kw IS NOT NULL
       ),
       par_transfo AS (
         SELECT t.transfo_id,
                t.code_actif AS code,
                COUNT(ps.point_id)::int AS n_clients,
                COALESCE(SUM(ps.puiss_souscrite_kw), 0)::numeric AS declare_kw,
